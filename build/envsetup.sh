...
- pixelgerrit:     A Git wrapper that fetches/pushes patch from/to LotusOS Gerrit Review.
...
- repopick:        Utility to fetch changes from LotusOS Gerrit.
...
function breakfast()
{
    ...
    for f in `/bin/ls vendor/lotus/vendorsetup.sh 2> /dev/null`
        do
            echo "including $f"
            . $f
        done
...
{
    if [ "$OUT" ] ; then
        ZIPPATH=`ls -tr "$OUT"/Lotus-*.zip | tail -1`
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
        fi
        ...
        if (adb shell getprop ro.lotus.device | grep -q "$LOTUS_BUILD"); then
...
EOF
            ...
            echo "The connected device does not appear to be $LOTUS_BUILD, run away!"
        fi
        return $?
    else
        echo "Nothing to eat"
...
function installboot()
{
    ...
    if (adb shell getprop ro.lotus.device | grep -q "$LOTUS_BUILD");
    then
...
function dopush()
{
    ...
    if (adb shell getprop ro.lotus.device | grep -q "$LOTUS_BUILD") || [ "$FORCE_PUSH" = "true" ];
    then
...
if [ -d $(gettop)/prebuilts/snapdragon-llvm/toolchains ]; then
    case `uname -s` in
        ...
        *)
            ...
            export SDCLANG_LTO_DEFS=$(gettop)/vendor/lotus/build/core/sdllvm-lto-defs.mk
            ;;
    esac
fi
