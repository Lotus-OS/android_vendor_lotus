...
lotus_local_manifest = ".repo/local_manifests/lotus.xml"
lotus_default_revision =  os.getenv('ROOMSERVICE_DEFAULT_BRANCH', 'ten')
lotus_dependencies = "lotus.dependencies"
org_manifest = "lotus-devices"  # leave empty if org is provided in manifest
...
def get_from_manifest(device_name):
    if os.path.exists(lotus_local_manifest):
        man = load_manifest(lotus_local_manifest)
...
def is_in_manifest(project_path):
    for man in (lotus_local_manifest, default_manifest):
...
def add_to_manifest(repos, fallback_branch=None):
    lm = load_manifest(lotus_local_manifest)
	...
	if 'branch' in repo:
	    ...
	    repo_branch=lotus_default_revision
	if 'remote' in repo:
...
def fetch_dependencies(repo_path, fallback_branch=None):
    ...
    dep_p = '/'.join((repo_path, lotus_dependencies))
...
def detect_revision(repo):
    ...
    if has_branch(result, lotus_default_revision):
