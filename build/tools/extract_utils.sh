...
function setup_vendor() {
    ...
    export ANDROIDBP="$LOTUS_ROOT"/"$OUTDIR"/Android.bp
...
function oat2dex() {
    ...
    if [ -z "$BAKSMALIJAR" ] || [ -z "$SMALIJAR" ]; then
        export BAKSMALIJAR="$LOTUS_ROOT"/prebuilts/tools-custom/common/smali/baksmali.jar
        export SMALIJAR="$LOTUS_ROOT"/prebuilts/tools-custom/common/smali/smali.jar
    fi

    if [ -z "$VDEXEXTRACTOR" ]; then
        export VDEXEXTRACTOR="$LOTUS_ROOT"/prebuilts/tools-custom/"${HOST,,}"-x86/bin/vdexExtractor
    fi

    if [ -z "$CDEXCONVERTER" ]; then
        export CDEXCONVERTER="$LOTUS_ROOT"/prebuilts/tools-custom/"${HOST,,}"-x86/bin/compact_dex_converter
    fi
